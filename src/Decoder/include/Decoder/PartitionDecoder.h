...
public:  
    ...
    void DecodePartition(Block4D reconstructedBlock,Hierarchical4DDecoder &entropyDecoder,std::array<double,2> disparityRange);
    double transformGain(std::array<int64_t,4> length);

    void DecodePartitionStep(Block4D reconstructedBlock,std::array<int64_t,4> position, std::array<int64_t,4> length, Hierarchical4DDecoder &entropyDecoder,std::array<double,2> disparityRange);
};
